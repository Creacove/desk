...
def replace_all(path, old, new, minimum=1):
    ...
    found = text.count(old)
    if found < minimum:
        raise SystemExit(f"{path}: expected at least {minimum} occurrence(s), found {found}: {old[:120]!r}")
...
def sub(path, pattern, replacement, count=1, flags=re.S):
    ...
    if n != count:
        raise SystemExit(f"{path}: regex replacement expected {count}, got {n}: {pattern[:120]!r}")
    write(path, next_text)


# Browser types: canonical song documents are first-class Manager work products.
replace_all(
...
# Backend created-work type carries the same document receipt metadata.
replace(
...
# Tool selection: EPK/press kit is a document, never an implicit share package.
agent_path = "supabase/functions/_shared/manager-conversation/agentLoop.ts"
replace(
    agent_path,
...
if 'export function managerConversationExplicitlyRequestsDecisionPackage' not in text:
    insertion = '''export function managerConversationExplicitlyRequestsDecisionPackage(input: {\n  body: string;\n  contextAnswers?: Array<{ questionKey: string; answer: string }>;\n}) {\n  const text = `${input.body} ${(input.contextAnswers ?? []).map((answer) => `${answer.questionKey} ${answer.answer}`).join(" ")}`\n    .replace(/[_-]+/g, " ")\n    .toLowerCase();\n  return /\\bdecision package\\b/.test(text);\n}\n\n'''
    if marker not in text:
        raise SystemExit("agentLoop: insertion marker missing")
    write(agent_path, text.replace(marker, insertion + marker, 1))

# Structured document preparation is exported so an unsaved generated draft can survive persistence failure.
draft_path = "supabase/functions/_shared/songDocumentDraft.ts"
replace(
    draft_path,
...
# Both the RPC and SQL fallback receipts include the unresolved dependency ledger.
text = read(draft_path)
needle = '      schemaVersion: "song_document_v2",\n    };'
if text.count(needle) < 2:
    raise SystemExit(f"songDocumentDraft: expected at least two receipt sites, found {text.count(needle)}")
write(draft_path, text.replace(needle, '      schemaVersion: "song_document_v2",\n      missingInputs: structure.missingInputs,\n    };'))

# Persistence: quality failures go back to the agent for repair; database failures preserve the draft in chat.
tool_path = "supabase/functions/_shared/manager-conversation/toolExecutor.ts"
replace(
    tool_path,
...
new_create_document = r'''async function createFocusedSongDocument(db: SupabaseLike, input: ManagerToolInput, args: Record<string, unknown>) {
...
sub(
    tool_path,
    r'async function createFocusedSongDocument\(db: SupabaseLike, input: ManagerToolInput, args: Record<string, unknown>\) \{.*?\n\}\n\nasync function prepareFocusedReleaseSharePackage',
    new_create_document,
)

# Shared mapping from document tool receipts to UI work products.
Path("supabase/functions/_shared/manager-conversation/documentWork.ts").write_text(r'''import type { ManagerConversationCreatedWork } from "../openaiManagerConversation.ts";
...
# Non-stream Manager: collect all document results and hard-gate decision packages.
normal_path = "supabase/functions/manager-conversation/index.ts"
replace(normal_path, '  managerConversationRequiresCanonicalDocumentTool,\n  runManagerAgentLoop,', '  managerConversationExplicitlyRequestsDecisionPackage,\n  managerConversationRequiresCanonicalDocumentTool,\n  runManagerAgentLoop,')
replace(normal_path, 'import { executeManagerConversationTool } from "../_shared/manager-conversation/toolExecutor.ts";', 'import { executeManagerConversationTool } from "../_shared/manager-conversation/toolExecutor.ts";\nimport { songDocumentWorkItems, upsertManagerCreatedWork, type ManagerToolResult } from "../_shared/manager-conversation/documentWork.ts";')
replace(normal_path, '    const { output, usage, responseId, toolTrace, toolCreatedWork } = await callOpenAIManagerConversation(', '    const { output, usage, responseId, toolTrace, toolCreatedWork, documentToolResults } = await callOpenAIManagerConversation(')
replace(
    normal_path,
...
replace(normal_path, '  const playbookInstructions = getPlaybooksInstructions(playbookKeys);\n  const toolCreatedWork: ManagerConversationOutput["createdWork"] = [];', '  const playbookInstructions = getPlaybooksInstructions(playbookKeys);\n  const toolCreatedWork: ManagerConversationOutput["createdWork"] = [];\n  const documentToolResults: ManagerToolResult[] = [];')
replace(normal_path, '    executeTool: (name, args) => executeManagerConversationTool(db, toolInput, name, args),', '    executeTool: async (name, args) => {\n      const toolResult = await executeManagerConversationTool(db, toolInput, name, args);\n      if (name === "create_focused_song_document") documentToolResults.push({ tool: name, result: toolResult });\n      return toolResult;\n    },')
replace(normal_path, '    toolTrace: result.toolTrace,\n    toolCreatedWork,\n  };', '    toolTrace: result.toolTrace,\n    toolCreatedWork,\n    documentToolResults,\n  };')
replace(normal_path, 'async function persistDecisionPackageOutput(db: any, input: ManagerConversationInput, conversationId: string, runId: string, output: ManagerConversationOutput) {\n  if (output.actionPolicy !== "create_decision_package") return null;', 'async function persistDecisionPackageOutput(db: any, input: ManagerConversationInput, conversationId: string, runId: string, output: ManagerConversationOutput) {\n  if (output.actionPolicy !== "create_decision_package" || !managerConversationExplicitlyRequestsDecisionPackage(input)) return null;')
replace(normal_path, '      artifactKind: item.artifactKind === "task_draft" ? "task_draft" : undefined,\n      content: item.content ? String(item.content) : undefined,\n      managerOutputId: item.managerOutputId ? String(item.managerOutputId) : undefined,', '      artifactKind: item.artifactKind === "task_draft" || item.artifactKind === "song_document" ? item.artifactKind : undefined,\n      content: item.content ? String(item.content) : undefined,\n      managerOutputId: item.managerOutputId ? String(item.managerOutputId) : undefined,\n      musicItemId: item.musicItemId ? String(item.musicItemId) : undefined,\n      documentType: item.documentType ? String(item.documentType) : undefined,\n      readiness: item.readiness === "ready" || item.readiness === "needs_review" || item.readiness === "save_failed" ? item.readiness : undefined,\n      missingInputs: Array.isArray(item.missingInputs) ? item.missingInputs.map((entry: unknown) => String(entry || "").trim()).filter(Boolean).slice(0, 20) : [],')

# Stream Manager: document generation is not release-success and all requested docs survive as separate work.
stream_path = "supabase/functions/manager-conversation-stream/index.ts"
replace(stream_path, '  managerConversationRequiresCanonicalDocumentTool,\n  runManagerAgentLoop,', '  managerConversationExplicitlyRequestsDecisionPackage,\n  managerConversationRequiresCanonicalDocumentTool,\n  runManagerAgentLoop,')
replace(stream_path, 'import { executeManagerConversationTool } from "../_shared/manager-conversation/toolExecutor.ts";', 'import { executeManagerConversationTool } from "../_shared/manager-conversation/toolExecutor.ts";\nimport { isReleaseSuccessArtifactTool, songDocumentWorkItems, upsertManagerCreatedWork } from "../_shared/manager-conversation/documentWork.ts";')
replace(
    stream_path,
...
replace(stream_path, '      if (!isReleaseSuccessTool(name) && !isOpportunityTool(name)) return executeManagerConversationTool(db, toolInput, name, args);', '      if (!isReleaseSuccessTool(name) && !isOpportunityTool(name) && !isDocumentTool(name)) return executeManagerConversationTool(db, toolInput, name, args);')
replace(stream_path, '          operation: isOpportunityTool(name) ? "release_opportunity_tool" : "release_success_tool",\n          source: "edge",\n          publicMessage: isOpportunityTool(name) ? "Release research could not be completed safely." : "Release materials could not be checked.",', '          operation: isDocumentTool(name) ? "song_document_tool" : isOpportunityTool(name) ? "release_opportunity_tool" : "release_success_tool",\n          source: "edge",\n          publicMessage: isDocumentTool(name) ? "The song document needs a retry." : isOpportunityTool(name) ? "Release research could not be completed safely." : "Release materials could not be checked.",')
replace(stream_path, '            stage: isOpportunityTool(name) ? opportunityToolStage(name) : name,', '            stage: isDocumentTool(name) ? "document_generation" : isOpportunityTool(name) ? opportunityToolStage(name) : name,')
sub(
    stream_path,
    r'function isReleaseSuccessTool\(tool: string\) \{\n  return tool === "read_focused_release_success"\n    \|\| tool === "propose_focused_release_date_change"\n    \|\| tool === "create_focused_song_document";\n\}\n\nfunction isOpportunityTool\(tool: string\) \{\n  return tool === "query_focused_release_opportunities"\n    \|\| tool === "save_focused_release_opportunities"\n    \|\| tool === "record_focused_release_opportunity_outcome"\n    \|\| tool === "create_focused_song_document";\n\}',
...
replace(stream_path, 'async function persistDecisionPackageOutput(db: any, input: ManagerConversationInput, conversationId: string, runId: string, output: ManagerConversationOutput) {\n  if (output.actionPolicy !== "create_decision_package") return null;', 'async function persistDecisionPackageOutput(db: any, input: ManagerConversationInput, conversationId: string, runId: string, output: ManagerConversationOutput) {\n  if (output.actionPolicy !== "create_decision_package" || !managerConversationExplicitlyRequestsDecisionPackage(input)) return null;')
replace(stream_path, '    artifactKind: item.artifactKind === "task_draft" ? "task_draft" : undefined,\n    content: item.content ? String(item.content) : undefined,\n    managerOutputId: item.managerOutputId ? String(item.managerOutputId) : undefined,', '    artifactKind: item.artifactKind === "task_draft" || item.artifactKind === "song_document" ? item.artifactKind : undefined,\n    content: item.content ? String(item.content) : undefined,\n    managerOutputId: item.managerOutputId ? String(item.managerOutputId) : undefined,\n    musicItemId: item.musicItemId ? String(item.musicItemId) : undefined,\n    documentType: item.documentType ? String(item.documentType) : undefined,\n    readiness: item.readiness === "ready" || item.readiness === "needs_review" || item.readiness === "save_failed" ? item.readiness : undefined,\n    missingInputs: Array.isArray(item.missingInputs) ? item.missingInputs.map((entry: unknown) => String(entry || "").trim()).filter(Boolean).slice(0, 20) : [],')

# Requested artifacts are rendered as documents with usable fallback draft content.
legacy_path = "src/features/manager/ManagerScreensLegacy.tsx"
old_card_pattern = r'''function DocumentResultCard\(\{\n  item,\n  onOpenCreatedWork,\n\}: \{\n  item: WorkItem;\n  onOpenCreatedWork: \(type: "music_item" \| "mission" \| "task", id\?: string, destination\?: CreatedWorkDestination\) => void \| Promise<void>;\n\}\) \{\n  return \(\n    <article data-testid="manager-document-result" className="flex items-center gap-3 border-l-2 border-foreground/12 py-1 pl-4">.*?\n  \);\n\}'''
new_card = r'''function DocumentResultCard({
  item,
  onOpenCreatedWork,
}: {
  item: WorkItem;
...
sub(legacy_path, old_card_pattern, new_card)

# Regression tests for partial-data documents and the release-review bug.
Path("src/manager-artifact-resilience.test.ts").write_text(r'''import { describe, expect, it } from "vitest";
...
# One-shot implementation files do not belong in the product branch.
for temporary in [
    ".github/workflows/implement-premium-manager-artifacts.yml",
    "scripts/implement-premium-manager-artifacts.py",
]:
    path = Path(temporary)
    if path.exists():
        path.unlink()
