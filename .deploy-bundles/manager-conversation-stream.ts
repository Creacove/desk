  ...
  const stream = new ReadableStream({
    async start(controller) {
      const emit = (event) => {
        ...
        try {
          eventIndex += 1;
          controller.enqueue(encoder2.encode(`id: ${eventIndex}
...
`));
      ...
      try {
        ...
        emit({
          ...
          conversation: {
            id: conversationId,
            topic: input.conversationId ? void 0 : titleFromBody(input.body),
            status: "Manager is thinking",
            prompt: input.body.trim(),
            messages: [
              toMessageViewModel(artistMessage)
            ],
            createdWork: []
          }
        });
        ...
        emit({
          ...
          runId,
          tool: "manager-router",
          label: "Preparing Manager answer",
          status: "running"
        });
        ...
        if (derivedProposal && input.musicSubject?.type === "music_item" && !releaseSuccessToolResults.some((item) => item.tool === "propose_focused_release_date_change")) {
          const proposalResult = await executeManagerConversationTool(db, {
            ...input,
            conversationId,
            runId: runId ?? void 0,
        ...
        const persistedDocument = documentToolResult?.result && isRecord10(documentToolResult.result) && documentToolResult.result.status === "drafted" ? documentToolResult.result : documentToolResult ? void 0 : await persistFocusedSongDocumentDraft(db, input, runId, output.responseBody, Boolean(output.contextQuestions.length));
        const documentWork = persistedDocument ? songDocumentCreatedWork(input, persistedDocument) : void 0;
        ...
        output.createdWork = documentWork ? upsertServerCreatedWork(baseCreatedWork, documentWork) : baseCreatedWork;
        ...
        for (const work of output.createdWork) {
          emit({
            ...
            runId,
            artifact: normalizeCreatedWorkItem(work),
            refresh: refreshHintForCreatedWork(work)
          });
        }
        ...
        emit({
          ...
          refresh: output.createdWork.some((work) => work.type === "mission" || work.type === "task") ? refreshHintForCreatedWorkItems(output.createdWork) : {
            conversations: false,
            music: Boolean(finalMusicSubject)
          }
        });
      } catch (error) {
        const failure = classifyManagerConversationError(error);
        console.error("manager-conversation-stream failed", {
          runId,
          failureStage,
...
async function callOpenAIManagerConversation(db, input, context, previousResponseId, playbookKeys, conversationId, runId, onToolEvent) {
  ...
  const result = await runManagerAgentLoop({
    ...
    executeTool: async (name, args) => {
      ...
      try {
        const result2 = await executeManagerConversationTool(db, toolInput, name, args);
        releaseSuccessToolResults.push({
          tool: name,
          result: result2
        });
...
function isReleaseSuccessTool(tool) {
  return tool === "read_focused_release_success" || tool === "propose_focused_release_date_change" || tool === "create_focused_song_document";
}
function isOpportunityTool(tool) {
  return tool === "query_focused_release_opportunities" || tool === "save_focused_release_opportunities" || tool === "record_focused_release_opportunity_outcome" || tool === "create_focused_song_document";
}
function opportunityToolStage(tool) {
...
async function streamReleaseSuccessArtifacts(db, input, conversationId, runId, toolResults, emit) {
  let latestPacket = null;
  for (const toolResult of toolResults) {
    const result = isRecord10(toolResult.result) ? toolResult.result : {};
    if (isRecord10(result.packet)) latestPacket = result.packet;
    const artifact = releaseSuccessArtifactFromToolResult(toolResult, input, conversationId, latestPacket);
    ...
    for (const nextArtifact of artifacts) {
      try {
        ...
        emit({
          ...
          conversationId,
          runId: runId ?? void 0,
          artifact: failedReleaseSuccessArtifact(nextArtifact, errorEventId ?? void 0),
          refresh: {
            music: true,
            missions: Boolean(nextArtifact.missionId),
            desk: true
...
function releaseSuccessArtifactFromToolResult(toolResult, input, conversationId, latestPacket) {
  if (input.musicSubject?.type !== "music_item") return null;
  if (!isReleaseSuccessTool(toolResult.tool)) return null;
  const result = isRecord10(toolResult.result) ? toolResult.result : {};
  const packet = isRecord10(result.packet) ? result.packet : latestPacket;
  const musicItem = isRecord10(packet?.musicItem) ? packet.musicItem : {};
  ...
  const mission = isRecord10(packet?.mission) ? packet.mission : {};
  const releasePlan = isRecord10(packet?.releasePlan) ? packet.releasePlan : {};
  ...
  const base = {
    ...
    subject: {
      title,
      itemType,
      ...stringValue(packet?.approvedReleaseDate) ? {
        approvedReleaseDate: stringValue(packet?.approvedReleaseDate)
      } : {}
    }
  };
  if (toolResult.tool === "read_focused_release_success" && result.status === "found" && isRecord10(result.packet)) {
    return {
      ...base,
      state: "assessed",
      ...isRecord10(result.packet.assessment) ? {
        assessment: result.packet.assessment
      } : {}
    };
  }
  if (toolResult.tool === "propose_focused_release_date_change" && result.status === "proposed" && isRecord10(result.request)) {
    const request = result.request;
    return {
      ...base,
      state: "proposed",
      requestId: stringValue(request.requestId) || void 0,
      previewHash: stringValue(request.previewHash) || void 0,
      idempotencyKey: stringValue(request.idempotencyKey) || void 0,
      ...isRecord10(request.preview) ? {
...
function investigatingReleaseSuccessArtifact(musicItemId, conversationId, title) {
  return {
    id: `release-success:${conversationId}:${musicItemId}`,
    musicItemId,
    state: "investigating",
    subject: {
      title: title || "Attached song",
      itemType: "song"
    }
  };
}
function failedReleaseSuccessArtifact(base, reference) {
  return {
    ...base,
    state: "failed",
    error: {
...
async function persistReleaseSuccessArtifact(db, input, conversationId, runId, artifact) {
  const { data: existingRunOutput, error: existingRunError } = runId ? await db.from("manager_outputs").select("id").eq("account_id", input.accountId).eq("artist_workspace_id", input.artistWorkspaceId).eq("artist_id", input.artistId).eq("output_type", "release_success_assessment").eq("created_from_run_id", runId).maybeSingle() : {
    data: null,
    error: null
  };
  ...
  const { error: staleError } = await db.from("manager_outputs").update({
    is_current: false
  }).eq("account_id", input.accountId).eq("artist_workspace_id", input.artistWorkspaceId).eq("artist_id", input.artistId).eq("conversation_id", conversationId).eq("output_type", "release_success_assessment").eq("subject_type", "music_item").eq("subject_id", artifact.musicItemId).eq("is_current", true);
  if (staleError) throw staleError;
  const assessment = isRecord10(artifact.assessment) ? artifact.assessment : {};
  const recommendation = isRecord10(assessment.recommendation) ? assessment.recommendation : {};
  const gates = [
    ...isRecord10(assessment.foundation) && Array.isArray(assessment.foundation.gates) ? assessment.foundation.gates : [],
    ...isRecord10(assessment.campaign) && Array.isArray(assessment.campaign.gates) ? assessment.campaign.gates : []
  ];
  const outputPayload = {
    account_id: input.accountId,
    artist_workspace_id: input.artistWorkspaceId,
    ...
    supporting_evidence_json: gates.flatMap((gate) => Array.isArray(gate?.evidence) ? gate.evidence : []).slice(0, 50),
    render_json: artifact,
    schema_version: "release-success-artifact-v1",
    is_current: true,
    created_from_run_id: runId
  };
  const outputQuery = existingRunOutput?.id ? db.from("manager_outputs").update(outputPayload).eq("id", existingRunOutput.id) : db.from("manager_outputs").insert(outputPayload);
...
function releaseSuccessArtifactSummary(artifact) {
...
async function persistPreflightFailureSafe(db, input, conversationId, failureStage, errorMessage) {
  try {
...
function managerToolLabel(tool) {
...
function normalizeCreatedWorkItem(item) {
  return {
...
function songDocumentCreatedWork(input, persistedDocument) {
  const musicItemId = input.musicSubject?.type === "music_item" ? input.musicSubject.id : "";
  const title = typeof persistedDocument.title === "string" && persistedDocument.title.trim() ? persistedDocument.title.trim() : "Release documents";
  if (!musicItemId) return void 0;
  return {
    type: "music_item",
    id: musicItemId,
    title,
...
function upsertServerCreatedWork(current, next) {
  const key = `${next.type}:${next.id ?? next.title}`;
  return [
    ...current.filter((item) => `${item.type}:${item.id ?? item.title}` !== key),
    next
  ];
}
...
function refreshHintForCreatedWork(work) {
  ...
  if (work.type === "task") return {
    missions: true,
    missionIds: work.parentMissionId ? [
      work.parentMissionId
    ] : [],
    taskIds: work.id ? [
      work.id
    ] : [],
  ...
  if (work.type === "music_item") return {
    music: true
  };
  return {};
}
function refreshHintForCreatedWorkItems(items) {
  return {
    conversations: false,
    music: items.some((work) => work.type === "music_item"),
    missions: true,
    missionIds: uniqueStrings(items.flatMap((work) => work.type === "mission" ? [
      work.id
    ] : work.parentMissionId ? [
      work.parentMissionId
    ] : [])),
