      ...
      try {
        ...
        if (existingRun?.status === "completed") {
          emit({
            ...
            conversation: await loadReplayConversation(db, input, existingRun.conversation_id),
            refresh: {}
          });
          return;
        }
        if (existingRun && managerConversationRunIsActive(existingRun)) {
          emit({
            type: "error",
            message: "Manager is still processing this request. Try again in a moment.",
        ...
        if (run.state === "completed") {
          emit({
            type: "conversation.completed",
            conversation: await loadReplayConversation(db, input, conversationId),
            refresh: {}
          });
          return;
        }
        ...
        const workspaceCreatedWork = reconcileManagerCreatedWork(toolCreatedWork).filter((work) => work.artifactKind !== "song_document");
        const createdSongWorkspace = workspaceCreatedWork.some((work) => work.type === "music_item") && workspaceCreatedWork.some((work) => work.type === "mission");
...
async function insertConversationMessage(db, input, conversationId, message) {
  ...
  if (managerRunId) {
    const { data: existing, error: existingError } = await db.from("conversation_messages").select("id,conversation_id,speaker,label,body,metadata,created_at").eq("manager_synthesis_run_id", managerRunId).eq("speaker", "manager").maybeSingle();
    if (existingError) throw existingError;
    if (existing) return existing;
  }
...
async function resolveArtistMessageForRun(db, input, conversationId, message) {
  if (!input.retryMessageId && input.requestId) {
    const { data: existing, error: existingError } = await db.from("conversation_messages").select("id,conversation_id,speaker,label,body,metadata,created_at").eq("account_id", input.accountId).eq("artist_workspace_id", input.artistWorkspaceId).eq("artist_id", input.artistId).eq("conversation_id", conversationId).eq("speaker", "artist").eq("request_id", input.requestId).maybeSingle();
    if (existingError) throw existingError;
    if (existing) return existing;
  }
...
function isOpportunityTool(tool) {
  return tool === "query_focused_release_opportunities" || tool === "save_focused_release_opportunities" || tool === "record_focused_release_opportunity_outcome";
}
...
function normalizeCreatedWorkItem(item) {
  return {
    ...
    missingInputs: Array.isArray(item.missingInputs) ? item.missingInputs.map((value) => String(value || "").trim()).filter(Boolean) : void 0,
