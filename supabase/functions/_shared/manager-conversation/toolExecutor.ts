...
async function readFocusedReleaseSuccess(db: SupabaseLike, input: ManagerToolInput) {
  ...
  const packet: ReleaseSuccessPacket & Record<string, unknown> = {
    ...
    metadata: factFromValue(releaseSuccess.metadata) ?? { state: "unknown", source: "release_success_packet" },
    credits: creditsFact(musicCredits),
    splits: splitsFact(musicSplits),
    clearances: factFromValue(releaseSuccess.clearances ?? metadata.clearances ?? identity.rights_state) ?? { state: "unknown", source: "release_success_packet" },
    identifiers: identifiersFact(musicIdentifiers),
    distributor: factFromValue(releaseSuccess.distributor ?? metadata.distributor) ?? assetFact(musicAssets, "distributor_export") ?? { state: "unknown", source: "release_success_packet" },
    campaign,
