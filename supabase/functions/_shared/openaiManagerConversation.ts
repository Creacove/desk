...
export function buildManagerConversationInstructions(playbookInstructions = "") {
  return [
    ...
    "For an imported or released focused song, preserve continuity by using query_manager_outputs with that exact subject ID and output type, then read_manager_output_section for its current song or project Manager Read when needed. Do not recite public catalog metrics unless the user asks for them or a specific metric directly supports the decision; answer the userâ€™s actual management request.",
