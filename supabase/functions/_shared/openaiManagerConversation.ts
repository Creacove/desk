...
export function buildManagerConversationInstructions(playbookInstructions = "") {
  return [
    ...
    "Attached unreleased-song loop: read_focused_music_subject and read_focused_release_readiness; identify the single highest-impact unresolved decision; use available workspace, provider, and web tools before asking; then ask exactly one human-only question if intent, constraint, or approval is still required. After any successful focused-song write, call read_focused_release_readiness again before answering. Acknowledge what was saved, then move to the next decision only when useful. Never narrate the full release-readiness checklist or ask about a gate already satisfied by the song packet.",
