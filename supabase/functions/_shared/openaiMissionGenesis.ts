...
export const MISSION_GENESIS_PROMPT_VERSION = "mission-genesis-grounded-v4";
...
const sharedInstructions = [
  ...
  "Use the packet's existing Manager diagnosis as internal context; do not invent the first strategy or repeat internal field names in visible copy. Before authoring a mission, use packet.managerIntelligence's diagnosis, mission implications, and careerConditionDiagnosis.",
  ...
  "Visible copy contract: write for an artist or manager who needs to decide or act now. Use plain, direct language that an artist or manager can understand on first read. Say what is happening, why it matters, and what happens next; avoid internal planning language or abstract labels when a concrete sentence will do.",
  "Mission and task titles must name a concrete decision or action. Include the relevant record, project, market, or partner when the packet supports it, and avoid titles that only describe a vague goal such as improving strategy or building alignment.",
  "permissionRequests.risk must be one plain sentence naming the specific possible consequence of this exact action if it goes ahead or is delayed, using only facts supported by the packet. Never invent an outcome, claim an action will cause errors without support, or use a generic severity label. If no specific consequence is supported, say plainly that the current information does not show one.",
  "Use the same standard for tasks.riskIfLate: write one concrete sentence about what the artist or team may lose or what decision may be delayed if the task is late. Do not invent a consequence that the packet does not support.",
  ...
  "Before activation, reject a generic mission, marketing-default mission, mixed-objective mission, system-facing tasks, vague checkpoints, or any mission not tied to one career condition.",
  "One mission equals one career-management workstream for one career condition. A mission cannot mix long-term career direction, team operations, and campaign execution.",
  ...
  "Think in terms of career leverage, how demand is built, and the artist's long-term place — not marketing checklists. An elite manager asks: what is the right next move to build this specific artist's leverage, audience, and commercial position? Which move should be left out for now? What creates asymmetric career value?",
  ...
  "Do not default to a 90-day timeframe. Timeline must reflect the true scope of work and any confirmed dates. Use weeks or months for most tasks. A three-month market expansion is three months. A DSP pitch cycle is 6–8 weeks. A brand partnership is 2–4 months of outreach and negotiation. A release campaign is 8–12 weeks of coordinated work. Hardcoding '7 days' for every task is WRONG.",
  ...
  "Use packet.recommendedMissionPatterns only when they fit the user's current request. An empty recommendation list is a valid signal; do not invent a long-term career direction or data-upload mission to fill it.",
