...
async function requestOpenAIMissionGenesis(instructions: string, context: unknown) {
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model: Deno.env.get("OPENAI_MISSION_GENESIS_MODEL") || Deno.env.get("OPENAI_MANAGER_REASONING_MODEL") || Deno.env.get("OPENAI_SUMMARY_MODEL") || "gpt-5-mini",
...
async function createUsageEvent(db: any, input: MissionGenesisInput, runId: string) {
  const { data, error } = await db
    ...
    .insert({
      ...
      provider: "openai",
      model_or_tool: Deno.env.get("OPENAI_MISSION_GENESIS_MODEL") || Deno.env.get("OPENAI_MANAGER_REASONING_MODEL") || Deno.env.get("OPENAI_SUMMARY_MODEL") || "gpt-5-mini",
