...
async function buildArtistOperatingPacket(db: any, input: MissionGenesisInput) {
  ...
  const [profile, evidence, musicItems, musicProjects, memory, agentReports, missions, tasks, sources, managerPackets] = await Promise.all([
    ...
    selectMany(db, "tasks", "id,mission_id,primary_checkpoint_id,title,owner_role,work_mode,status,assignee_user_id,assignment_reason,assignment_version,purpose,evidence_needed,completion_expectation,risk_if_late", input, MISSION_GENESIS_PACKET_LIMITS.tasks),
