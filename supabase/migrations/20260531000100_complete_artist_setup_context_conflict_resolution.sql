-- Resolve return-column name ambiguity in complete_artist_setup_context for
-- databases that already applied the original setup completion RPC migration.
...
begin
  ...
  if nullif(trim(p_stage), '') is null
    or nullif(trim(p_home_market), '') is null
    or coalesce(array_length(p_genres, 1), 0) = 0
    or nullif(trim(p_artist_direction), '') is null
    or nullif(trim(p_budget_context), '') is null then
    raise exception 'artist stage, home market, genre, artist direction, and budget are required';
  end if;
  ...
  set stage = trim(p_stage),
      home_market = trim(p_home_market),
      genres = p_genres,
  ...
  values (
    ...
    jsonb_build_object(
      'required_fields', array['stage','home_market','genres','artist_direction','budget_context'],
