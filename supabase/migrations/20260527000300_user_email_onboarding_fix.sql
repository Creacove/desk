-- Fix workspace onboarding for users with pre-existing orphaned user profiles.
-- If a user signed up previously, deleted their account in Auth, and signed up
-- again with the same email, they will have a new user ID but their old email
-- will still exist in the public.users table. This causes a unique key violation on email.
--
-- This migration updates the create_initial_artist_workspace function to delete
-- any orphaned public.users row with the same email but a different id before inserting.
...
begin
  ...
  -- Clean up any orphaned user profiles with the same email but a different id
