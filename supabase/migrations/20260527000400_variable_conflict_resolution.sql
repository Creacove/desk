-- Resolve variable conflict ambiguity in create_initial_artist_workspace.
-- In some Postgres configurations, referring to column names that match return
-- table columns (like account_id, artist_workspace_id, etc.) inside the PL/pgSQL
-- function body triggers a "column reference is ambiguous" error.
--
-- This migration redefines the function with the `#variable_conflict use_column`
-- directive to explicitly dictate name resolution priority.
