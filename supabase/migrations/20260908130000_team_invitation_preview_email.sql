-- Add the invited email to the safe, unauthenticated preview without changing
-- the token-hash-only lookup or exposing account/workspace identifiers.
...
begin
  ...
  if nullif(pg_catalog.btrim(coalesce(team_name_value, '')), '') is null
     or nullif(pg_catalog.btrim(coalesce(artist_name_value, '')), '') is null
     or nullif(pg_catalog.btrim(coalesce(invitation_row.email, '')), '') is null then
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'invitedEmail', pg_catalog.lower(pg_catalog.btrim(invitation_row.email)),
