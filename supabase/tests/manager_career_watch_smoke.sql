...
do $$declare definition text;begin
  ...
  if to_regprocedure('public.begin_manager_career_watch_execution_v1(uuid,uuid,uuid,uuid)') is null then raise exception 'Career Watch execution lease claim is missing';end if;
  if to_regprocedure('public.finish_manager_career_watch_execution_v1(uuid,uuid,uuid,uuid,uuid,boolean,text)') is null then raise exception 'Career Watch lease-owned completion is missing';end if;
