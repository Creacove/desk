...
do $$declare definition text;begin
  ...
  if position('source_kind' in definition)=0 or position('recommended_decision' in definition)=0 or position('public.reviews' in definition)=0 then raise exception 'Career Watch does not route actionable evidence through the existing Manager review runtime';end if;
