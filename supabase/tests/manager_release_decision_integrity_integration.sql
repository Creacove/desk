...
begin
  ...
  values (v_artist_id, v_account_id, 'Otmos regression');
  insert into public.artist_workspaces (id, account_id, artist_id, name, status)
  values (v_workspace_id, v_account_id, v_artist_id, 'Otmos', 'active');
  ...
  insert into public.music_items (
    ...
    v_song_id, v_account_id, v_workspace_id, v_artist_id,
  ...
  insert into public.music_release_plans (
    ...
    v_release_plan_id, v_account_id, v_workspace_id, v_artist_id,
    v_song_id, v_mission_id, 'approved', date '2026-09-05', 0
  );
  ...
  insert into public.reviews (
    ...
    v_question_review_id, v_account_id, v_workspace_id, v_artist_id, v_mission_id,
  ...
  v_approval := public.approve_release_date_change(
    v_account_id,
    v_workspace_id,
    v_artist_id,
    v_request_id,
  ...
  if v_approval ->> 'approvedDate' is distinct from '2026-09-26' then
    raise exception 'approval result did not persist September 26: %', v_approval;
  end if;

  select planned_release_date into v_provider_date
  from public.music_items where id = v_song_id;
  if v_provider_date is distinct from date '2026-09-05' then
    raise exception 'provider/imported release date was rewritten: %', v_provider_date;
  end if;

  select approved_release_date into v_approved_date
  from public.music_release_plans where id = v_release_plan_id;
  if v_approved_date is distinct from date '2026-09-26' then
    raise exception 'canonical release plan did not move to September 26: %', v_approved_date;
  end if;
  ...
  from public.manager_question_requests where id = v_question_id;
  if v_question_status is distinct from 'superseded' then
    raise exception 'resolved release-date question survived approval: %', v_question_status;
  end if;

  select count(*) into v_active_fact_count
  from public.artist_operating_facts f
  where f.artist_workspace_id = v_workspace_id
    and f.artist_id = v_artist_id
    and f.fact_key = 'time.release_date'
    and f.scope_type = 'mission'
    and f.scope_key = 'mission:' || v_mission_id::text
    and f.status = 'active'
    and f.display_value = '2026-09-26';
  if v_active_fact_count <> 1 then
    raise exception 'expected exactly one canonical active release-date fact, got %', v_active_fact_count;
  end if;

  select count(*) into v_projection_count
  from public.memory_entries m
  where m.artist_workspace_id = v_workspace_id
    and m.source_type = 'canonical_release_plan'
    and m.source_id = v_release_plan_id;
  ...
  if v_projection_count <> 1
     or v_projection ->> 'effectiveReleaseDate' is distinct from '2026-09-26'
     or v_projection ->> 'musicItemId' is distinct from v_song_id::text then
  ...
  where r.artist_workspace_id = v_workspace_id
    and r.runtime_key = 'release-date-change:' || v_request_id::text || ':approved';
  if v_adaptive_review_count <> 1 then
    raise exception 'expected exactly one adaptive replan review after approval, got %', v_adaptive_review_count;
  end if;
  ...
  v_replay := public.approve_release_date_change(
    v_account_id,
    v_workspace_id,
    v_artist_id,
    v_request_id,
  ...
  if v_replay is distinct from v_approval then
  ...
  if v_active_fact_count <> 1 or v_projection_count <> 1 or v_adaptive_review_count <> 1 then
    raise exception 'approval replay duplicated canonical state: facts %, projections %, reviews %',
      v_active_fact_count, v_projection_count, v_adaptive_review_count;
  end if;
end;
$$;
